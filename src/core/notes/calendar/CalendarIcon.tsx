...
import { FaAmazon, FaAngry, FaAws, FaBaby, FaBirthdayCake, FaBookReader, FaBuilding, FaBus, FaCarSide, FaChalkboardTeacher, FaChess, FaCode, FaCross, FaDog, FaEuroSign, FaFileInvoiceDollar, FaGitSquare, FaGithub, FaGoodreads, FaGrinTongueWink, FaHeadSideCough, FaHiking, FaHome, FaInstagram, FaKey, FaLaptop, FaLinkedin, FaMoneyBillWave, FaPhone, FaPiggyBank, FaPodcast, FaPrayingHands, FaQuestionCircle, FaQuoteLeft, FaRss, FaScroll, FaSkype, FaSmile, FaSwimmingPool, FaTelegram, FaTired, FaTrain, FaTree, FaVenus, FaVoteYea, FaWpforms, FaYoutube, FaBible as IconoBible, FaEuroSign as IconoPayroll } from 'react-icons/fa';
...
import { GiAncientRuins, GiBank, GiChampions, GiChurch, GiCommercialAirplane, GiCook, GiCrownCoin, GiDeathSkull, GiDevilMask, GiEclipse, GiFarmer, GiFemaleLegs, GiFriedEggs, GiGluttony, GiHammerSickle, GiHealthDecrease, GiHealthIncrease, GiHealthNormal, GiKneeling, GiMiracleMedecine, GiPalmTree, GiPeaceDove, GiPerspectiveDiceThree, GiPrayerBeads, GiPumpkinMask, GiSoccerBall, GiSparkSpirit, GiStrong, GiTheaterCurtains, GiThermometerCold, GiThreeFriends, GiVillage, GiWheat } from 'react-icons/gi';
...
import { SiTinder as IconoTinder, SiDell, SiGooglemeet, SiInsomnia, SiLenovo, SiObsidian, SiOpenai, SiSamsung, SiSpotify, SiTwitter, SiWhatsapp } from 'react-icons/si';
...
export const iconData: IconData = {
  ...
  Economía: {
    ...
    finances: ({ size }: IconProps) => <FaMoneyBillWave size={size} className="red" />,
  ...
  Otros: {
    ...
    cold: ({ size }: IconProps) => <PiThermometerCold size={size} className="blue aws" />,
    comunism: ({ size }: IconProps) => <GiHammerSickle size={size} className="red comunism" />,
    doctor: ({ size }: IconProps) => <FaUserDoctor size={size} className="green" />,
    dream: ({ size }: IconProps) => <TbZzz size={size} className="blue cobalt" />,
    eclipse: ({ size }: IconProps) => <GiEclipse size={size} className="yellow" />,
    ...
    money: ({ size }: IconProps) => <FaMoneyBillWave size={size} className="red" />,
    ...
    rain: ({ size }: IconProps) => <BsFillCloudRainHeavyFill size={size} className="blue sky" />,
    repair: ({ size }: IconProps) => <MdBuild size={size} className="orange" />,
