...
export class GoodreadsReviewsApi extends GoodreadsApiBase {
    ...
    // SEPARAR ESTOS MÉTODOS A UNA CLASE APARTE

    private async fetchShelfRss(shelf: string): Promise<string | null> {
        const { goodreads_user, goodreads_apikey }: MyPluginSettings = this.getGoodreadsSettings();
        const url = `${GoodreadsApiBase.BASE_URL}/${GoodreadsReviewsApi.REVIEWS_RSS}`
            .replace('$userId', goodreads_user)
            .replace('$apikey', goodreads_apikey)
            .replace('$shelf', shelf);
    ...
    public async getToReadList() {
        const xmlString = await this.fetchShelfRss('to-read');
        if (!xmlString) return;
